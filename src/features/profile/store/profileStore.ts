...
interface ProfileStoreActions {
  ...
  updateNestedField: {
    workInfo: <K extends keyof User['workInfo']>(field: K, value: User['workInfo'][K]) => void
    communicationPref: <K extends keyof User['communicationPreference']>(
      field: K,
      value: User['communicationPreference'][K]
    ) => void
  }
  startEditing: () => void
  cancelEditing: () => void
    ...
    (set, get) => ({
      ...
      updateNestedField: {
        workInfo: (field, value) => {
          const { user } = get()
          if (!user?.workInfo) return
          set({
            user: {
              ...user,
              workInfo: { ...user.workInfo, [field]: value },
            },
            isEdited: true,
          })
        },
        communicationPref: (field, value) => {
          const { user } = get()
          if (!user?.communicationPreference) return
          set({
            user: {
              ...user,
      ...
      updateAvatar: async (file: File) => {
        set({ isSaving: true, error: null })
        try {
          const { avatar_url: avatarUrl } = await profileApi.updateAvatar(file)
          const { user } = get()
          if (user) {
            set({
              user: { ...user, avatarUrl },
