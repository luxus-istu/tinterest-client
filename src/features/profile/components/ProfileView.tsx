...
export function ProfileView() {
  const { displayUser, user, isLoading } = useProfile()
  ...
  const ageString = displayUser.age ? `${displayUser.age} лет` : 'возраст не указан'

  const interests = displayUser.interests.slice(0, 6)
    ...
    <main className="px-5">
      <header className="flex items-center justify-between pt-15">
        ...
        <Link href="/profile/settings">
          <svg className="h-12 fill-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
      ...
      <section className="bg-background flex flex-col items-start justify-center pt-10">
        <section>
          <div className="flex items-center justify-between gap-6">
            ...
            <section className="flex flex-col gap-2">
              ...
              <div className="flex w-fit items-center justify-center gap-2 rounded-full bg-[#2C2C2E] px-2 py-1.5">
                ...
                <p className="text-white">{displayUser.communication_goal}</p>
              </div>
            </section>
          </div>
        </section>
        <section className="flex w-full flex-col gap-4 pt-4 pb-10">
          <h2 className="text-2xl font-bold">Ваша анкета</h2>
          <div className="rounded-4xl bg-[#0A0A0A] shadow-lg shadow-white/5">
            <div
              className="aspect-4/6 w-full rounded-4xl bg-cover bg-center p-6"
              style={{ backgroundImage: `url('${displayUser.avatarUrl}')` }}
            >
              <div className="flex justify-between">
                <section className="flex flex-col gap-2">
                  ...
                  <div className="flex w-fit items-center justify-center gap-2 rounded-full bg-[#2C2C2E] px-2 py-1.5">
                    ...
                    <p className="text-white">{displayUser.city || 'Город не указан'}</p>
                  </div>
                </section>
            ...
            <section className="flex flex-col gap-5 p-6 pb-14">
              <div className="flex flex-col gap-1.5">
                <h1 className="text-lg font-bold text-[#8E8E93]">Обо мне</h1>
                <p className="text-lg">{displayUser.about || 'Информация не указана'}</p>
              </div>
              <div className="flex flex-col gap-1.5">
                <h1 className="text-lg font-bold text-[#8E8E93]">Должность</h1>
                <p className="text-lg">{displayUser.job_title || 'Не указана'}</p>
              </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {interests.map((interest) => (
                    <div
                      ...
                      className="flex w-fit items-center justify-center gap-2 rounded-lg bg-[#2C2C2E] px-3.5 py-1.5"
                    >
                      <svg
                        className="h-5 fill-white"
                        xmlns="http://www.w3.org/2000/svg"
                      ...
                      <p className="text-white">{interest.name}</p>
                    </div>
                  ))}
                  {interests.length === 0 && (
                    <p className="col-span-3 text-gray-400">Интересы не указаны</p>
                  )}
                </div>
              </section>
            </section>
